// Gemeinsame Helfer fuer die Relay-Testscripts (test-api.mjs,
...
// Node-Binary: process.execPath kann in Sandbox-Umgebungen (mise) auf einen
// nicht mehr existierenden Pfad zeigen - dann ueber PATH auf 'node' fallen.
const NODE_BIN = existsSync(process.execPath) ? process.execPath : 'node';

const PORT = Number(process.env.PORT ?? 8099);
const BASE = `http://localhost:${PORT}`;
const WS_BASE = `ws://localhost:${PORT}`;
const RELAY_ENTRY = join(
...
export async function startServer(extraEnv = {}) {
	...
	const server = spawn(NODE_BIN, [RELAY_ENTRY], {
		cwd: dirname(RELAY_ENTRY),
