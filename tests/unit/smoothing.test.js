...
test('trifft an jedem Stützpunkt exakt den vorgegebenen Wert (monoton wachsend)', () => {
	...
	for (let p of pts)
		assert.ok(Math.abs(at(p.t) - p.v) < 1e-6, `at(${p.t}) sollte ${p.v} sein, war ${at(p.t)}`);
});
...
test('C¹-stetig an den Rändern (Naht zur konstanten Fortsetzung, keine Steigung)', () => {
	...
	assert.equal(
		derivJustOutside,
		0,
		'außerhalb der Stützpunkte muss die Fortsetzung exakt konstant sein',
	);

	let last = pts[pts.length - 1].t;
	let derivJustInsideEnd = (at(last) - at(last - h)) / h;
	assert.ok(
		Math.abs(derivJustInsideEnd) < 1e-2,
		`Randtangente am Ende nicht ~0: ${derivJustInsideEnd}`,
	);
...
test('globale Monotonie bleibt über die gesamte Kurve erhalten (nicht nur an Stützpunkten)', () => {
	...
	for (let t = pts[0].t - 2; t <= pts[pts.length - 1].t + 2; t += 0.01) {
		...
		assert.ok(
			val >= prev - 1e-9,
			`Monotonie verletzt bei t=${t.toFixed(2)}: ${val} < vorheriger Wert ${prev}`,
		);
		prev = val;
	}
});
...
test('ohne onlyChanges: Wertwiederholungen erzwingen "weiche Stufen" (Nulltangente an jedem Wiederholungspunkt)', () => {
	...
	assert.equal(
		(at(3 + h) - at(3 - h)) / (2 * h) < 0.05,
		true,
		'Tangente kurz vor dem Wertwechsel sollte ~0 sein (Stufen-Effekt)',
	);
});
...
test('mit onlyChanges: Wertwiederholungen werden verworfen, Übergang fließt statt zu stufen', () => {
	...
	assert.ok(
		derivAt3 > 0.05,
		`Steigung bei t=3 sollte spürbar > 0 sein (fließender Übergang), war ${derivAt3}`,
	);
...
test('computeSegmentBlend: kein Vorauseilen - für t knapp VOR einem Stützpunkt bleibt s praktisch 0', () => {
	...
	assert.ok(
		justBefore.s > 0.99,
		`s direkt vor dem Stützpunkt sollte nahe 1 sein (Ende SEINES EIGENEN Segments), war ${justBefore.s}`,
	);
});
...
test('buildDampedFilter: C¹-stetig an jedem Stützpunkt (keine Ableitungssprünge, per finiter Differenz)', () => {
	...
	for (let p of pts) {
		...
		assert.ok(
			...
			`Ableitungssprung bei t=${p.t}: links=${left.toFixed(6)}, rechts=${right.toFixed(6)}`,
		);
	}
});
	...
	for (let tau of [0.1, 1, 5, 20]) {
		...
		for (let t = -5; t <= 30; t += 0.5) {
			...
			assert.ok(
				...
				`TAU=${tau}: Wert ${v} bei t=${t} außerhalb [${vMin},${vMax}]`,
			);
		}
	}
});
...
test('buildDampedFilter: größeres TAU bedeutet trägere (langsamere) Reaktion', () => {
	...
	assert.ok(
		atFast(6) > atSlow(6),
		`bei TAU klein sollte der Wert schneller aufholen: fast=${atFast(6)}, slow=${atSlow(6)}`,
	);
});
...
test('buildDampedFilter: reagiert auf viele dicht aufeinanderfolgende Stützpunkte spürbar träger als buildMonotoneSpline (Kern des Bank-Zoom-Fixes)', () => {
	...
	assert.ok(
		lag > 0.3,
		`gedämpfter Filter sollte bei t=${t} spürbar hinter dem exakten Wert liegen, Differenz war nur ${lag.toFixed(3)}`,
	);
});
