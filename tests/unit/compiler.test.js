...
const BASE_CONFIG = {
    ...
    bankZoomThresholdPowers: 0, zoomSpeedCoef: 0.012,
...
test('compileSystem() ist eine reine Funktion: identische Config -> tief-gleiches Ergebnis', () => {
    ...
    assert.deepStrictEqual(r1.bank_pieces.map(p => [p.x, p.y, p.w, p.h]), r2.bank_pieces.map(p => [p.x, p.y, p.w, p.h]));
});
...
test('P_FINAL nähert sich mit wachsender Tiefe sqrt(2) monoton an (Kernzweck des Tools)', () => {
    ...
    for (let depth of [1, 2, 4, 6]) {
        ...
        assert.ok(error < prevError, `Tiefe ${depth}: Fehler ${error} sollte kleiner sein als bei geringerer Tiefe (${prevError})`);
        prevError = error;
    }
});
...
test('GLOBAL_TTM: Tick<->Zeit-Abbildung ist bijektiv und monoton auf [0, maxTick]', () => {
    ...
    for (let tick = 0; tick <= r.GLOBAL_TTM.maxTick; tick++) {
        ...
        assert.ok(time > lastTime, `Zeit bei Tick ${tick} (${time}) muss strikt größer sein als bei Tick ${tick - 1} (${lastTime})`);
        lastTime = time;
    }
});
...
test('Kompaktierung mit ungültigem compactionTransitionTicks (NaN) fällt auf Default 3 zurück statt zu crashen', () => {
    assert.doesNotThrow(() => compileSystem({ ...BASE_CONFIG, compactionEnabled: true, compactionTransitionTicks: NaN }));
});
