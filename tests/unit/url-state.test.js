...
const FAKE_COMPILED = {
	MAX_TIME: 10,
	GLOBAL_TTM: { maxTick: 20, tickToTime: (t) => t / 2, timeToTick: (t) => t * 2 },
};
...
test('parseConfigFromUrl(): Checkbox-Feld "compaction" wird als Boolean gelesen', () => {
	assert.deepStrictEqual(parseConfigFromUrl(new URLSearchParams('compaction=1')), {
		compactionEnabled: true,
	});
	assert.deepStrictEqual(parseConfigFromUrl(new URLSearchParams('compaction=0')), {
		compactionEnabled: false,
	});
});
...
test('parsePlaybackFromUrl(): "play" wird als Boolean gelesen, fehlt ohne Parameter', () => {
	assert.strictEqual(
		parsePlaybackFromUrl(new URLSearchParams('play=1'), FAKE_COMPILED).isPlaying,
		true,
	);
	assert.strictEqual(
		parsePlaybackFromUrl(new URLSearchParams(''), FAKE_COMPILED).isPlaying,
		undefined,
	);
});
