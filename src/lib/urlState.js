...
const CONFIG_FIELDS = [
    ...
    { key: 'zoomthresh', field: 'bankZoomThresholdPowers', parse: (v) => parseInt(v) || 0, format: (c) => String(c.bankZoomThresholdPowers) },
    { key: 'autozoom', field: 'autoZoomMinPx', parse: (v) => parseFloat(v) || 0, format: (c) => String(c.autoZoomMinPx) },
    { key: 'zoomspeed', field: 'zoomSpeedCoef', parse: (v) => parseFloat(v) || 0.012, format: (c) => String(c.zoomSpeedCoef) },
    { key: 'linewidth', field: 'lineWidth', parse: (v) => parseFloat(v) || 0, format: (c) => String(c.lineWidth) },
    { key: 'pause', field: 'pauseDuration', parse: (v) => parseFloat(v) || 0, format: (c) => String(c.pauseDuration) },
    { key: 'compaction', field: 'compactionEnabled', parse: (v) => v === '1', format: (c) => (c.compactionEnabled ? '1' : '0') },
    { key: 'speed', field: 'playSpeed', parse: (v) => parseFloat(v) || 2.0, format: (c) => String(c.playSpeed) },
    { key: 'transition', field: 'compactionTransitionTicks', parse: (v) => parseInt(v), format: (c) => String(c.compactionTransitionTicks) },
    { key: 'modeab', field: 'modeAB', parse: (v) => Math.max(0, Math.min(1, parseFloat(v) || 0)), format: (c) => c.modeAB.toFixed(4) },
];
...
export function parsePlaybackFromUrl(params, compiled) {
    ...
    if (params.has('time')) {
        ...
        let tick = Math.max(0, Math.min(compiled.GLOBAL_TTM.maxTick, Math.round(parseFloat(params.get('tick')) || 0)));
