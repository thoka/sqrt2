<script>
  ...
  let pct = $derived($compiledStore.MAX_TIME > 0 ? ($playbackStore.time / $compiledStore.MAX_TIME) * 100 : 0);
</script>

<button id="playBtn" title="Play/Pause" onclick={togglePlaying}>{$playbackStore.isPlaying ? '⏸' : '▶'}</button>
<input type="range" id="timeSlider" min="0" max={$compiledStore.MAX_TIME} step="0.01"
    value={$playbackStore.time} oninput={onSliderInput}
    style="background-image: linear-gradient(to right, #fff {pct}%, transparent {pct}%);">
<span id="timeReadout">
    {#if $compiledStore.GLOBAL_TTM}
        {$playbackStore.time.toFixed(1)} · {Math.round($compiledStore.GLOBAL_TTM.timeToTick($playbackStore.time))}/{$compiledStore.GLOBAL_TTM.maxTick}
    {:else}
