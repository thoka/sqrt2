<script>
  ...
  let counts = $derived.by(() => {
    ...
    for (let p of c.bank_pieces) {
      if (p.k < arr.length && time >= p.born_time && time < p.cut_time && time < p.taken_time) arr[p.k]++;
    }
    return arr;
  });
...
<div id="bankPanelInner" bind:this={innerEl}>
  {#each counts as count, k}
    ...
    <div class="bank-row" style="opacity: {count === 0 ? 0.35 : 1.0}">
      ...
      {:else}
        <div class="bank-bar"><div style="flex-grow:1; height:10px; border-radius:2px; background:{color}"></div></div>
      {/if}
