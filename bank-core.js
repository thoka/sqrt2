...
export function buildCompactionMap(pieces, axis) {
    ...
    let anchorIdx = 0;
    for (let i = 1; i < merged.length; i++) if (merged[i].mass > merged[anchorIdx].mass) anchorIdx = i;
...
export function computeCompactionAt(bank_pieces, tickValue, transitionTicks = DEFAULT_GAP_CLOSE_TRANSITION_TICKS) {
    let visible = bank_pieces.filter(p => tickValue >= p.born_time && tickValue < p.cut_time && tickValue < p.taken_time + GAP_CLOSE_DELAY_TICKS + transitionTicks);
    if (visible.length === 0) return { mapX: x => x, mapY: y => y, totalW: 1, totalH: 1, minX: 0, minY: 0 };
    let mapX = buildCompactionMap(visible, 'x');
    let mapY = buildCompactionMap(visible, 'y');
    return {
        mapX: mapX.compact, mapY: mapY.compact,
        totalW: mapX.totalOccupied, totalH: mapY.totalOccupied,
        ...
        minX: mapX.minCoord, minY: mapY.minCoord,
    };
}
...
export function computeCompactionWaypoints(bank_pieces, maxTick, transitionTicks = DEFAULT_GAP_CLOSE_TRANSITION_TICKS) {
    ...
    return allTicks.map(t => {
        ...
        return {
            t, mapX: comp.mapX, mapY: comp.mapY,
            totalW: comp.totalW, totalH: comp.totalH,
            minX: comp.minX, minY: comp.minY, z,
        };
    });
}
...
export function getSmoothedCompactedLogicalRect(piece, waypoints, time) {
    ...
    return blendLogicalRect(piece, waypoints, waypoints.map(wp => wp.t), time);
}
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = {
        ...
        computeCompactionWaypoints, compactedLogicalRectAt, getSmoothedCompactedLogicalRect,
        makeCompactedLogicalRectLookup, computeCompactionFitStates, applyCompactionFit,
