...
- `getSmoothedAutoZoomExp(time)` liefert den geglätteten Ziel-Exponenten für den aktuellen Zeitpunkt. **Nicht** der einfache Exponentialkern aus `getBankTransform()` (Checkpoints + `F(t)=exp(-k·(time-t))`, siehe Abschnitt 6.1) - der ist zwar stetig im WERT, aber NICHT in der ABLEITUNG: an jedem Checkpoint springt die Steigung sprunghaft (numerisch nachgewiesen, z.B. 0.000→0.250 exakt am Checkpoint). Bei einer Checkpoint pro Schale (potenziell Dutzende bis Hunderte) macht sich das als spürbares Ruckeln bemerkbar - `getBankTransform()` hat denselben Fehler, ist dort aber bisher nicht aufgefallen (weniger/subtiler). Fix hier: jeder Checkpoint-Sprung wird als Sprungantwort eines **kritisch gedämpften Systems 2. Ordnung** eingebracht (`h(τ) = 1 - exp(-k·τ)·(1+k·τ)` für `τ>0`) statt als einfacher Exponentialkern - diese Sprungantwort startet mit WERT UND STEIGUNG exakt Null, die Superposition mehrerer solcher Antworten ist dadurch über die gesamte Zeitachse C¹-stetig, auch an den Checkpoints selbst (geschlossen nachrechenbar, numerisch verifiziert: keine Steigungssprünge mehr an über 20 getesteten Checkpoints). Weiterhin kausal und für jeden Zeitpunkt exakt auswertbar (kein Zustand nötig) - funktioniert daher genauso beim Scrubben/Zurückspulen. Anders als beim Bank-Zoom ist hier **keine** Monotonie gewünscht/erzwungen - beim Zurückspulen soll sich das Ziel genauso glatt wieder verkleinern (verifiziert: symmetrisches Hoch-/Runterfahren vorwärts/rückwärts).
