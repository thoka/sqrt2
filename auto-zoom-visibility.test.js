...
function checkNoInvisibleDigit(BASE, N_MAX, thresholdPx, scale) {
    ...
    let { widthAt, computeAutoZoomTAB } = makeAutoZoomTools(axes, TOTAL_STEPS, N_MAX, BASE, P_FINAL, scale);
    ...
    let spline = buildMonotoneSpline(shellStart.map((t, S) => ({ t, v: axes[S].exp })), { onlyChanges: true });
    ...
    for (let S = 1; S < TOTAL_STEPS; S++) {
        for (let dt of [0, 0.01, 0.05, 0.2]) {
            ...
            let trueStep = 0;
            for (let s = 1; s < TOTAL_STEPS; s++) { if (time >= shellStart[s]) trueStep = s; else break; }
            ...
            assert.ok(actualWidth >= allowedWidth,
