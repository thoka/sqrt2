...
const main = async () => {
  ...
  check('host not counted against seats', host.msgs.some((m) => m.type === 'joined' && m.seats === 2 && m.occupied === 0));
  ...
  await wait(100);
  check('guest A joined', a.msgs.some((m) => m.type === 'joined' && m.role === 'guest'));
  check('guest B joined', b.msgs.some((m) => m.type === 'joined' && m.role === 'guest'));
  check('presence join broadcast', host.msgs.some((m) => m.type === 'presence' && m.event === 'join'));

  a.ws.send(JSON.stringify({ type: 'app', payload: { hello: 'world' } }));
  await wait(100);
  check('relay A->B (app)', b.msgs.some((m) => m.type === 'app' && m.payload?.hello === 'world'));
  check('relay A->host (app)', host.msgs.some((m) => m.type === 'app' && m.payload?.hello === 'world'));

  // 3) Dritter Gast -> seats_exhausted (Limit 2 Gaeste)
  const c = await openWs(token, 'guest', '1234');
  await wait(100);
  check('seat limit enforced (seats_exhausted)', c.ws._closed !== undefined && c.ws._closed.c !== undefined);
  c.ws.close?.();
  ...
  await wait(100);
  check('new pin accepted', newPin.msgs.some((m) => m.type === 'joined'));
  ...
  check('presence leave broadcast', hostPresence.length > beforeLeave && hostPresence.some((m) => m.event === 'leave'));
  ...
  w1.ws.close?.(); w2.ws.close?.(); w3.ws.close?.(); ok.ws.close?.();
