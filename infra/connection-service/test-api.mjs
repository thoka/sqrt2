...
const main = async () => {
  ...
  const checker = makeChecker(); const { check } = checker;

  // 1) Oeffentliche Endpunkte
  const health = await req('GET', '/health');
  check('health ok + version', health.status === 200 && health.json.ok === true && !!health.json.version);
  ...
  check('status page cors allowed origin', status.headers.get('access-control-allow-origin') === ALLOWED);
  ...
  check('cors preflight 204 + ACAO', pre.status === 204 && pre.headers.get('access-control-allow-origin') === ALLOWED);
  ...
  const mint = await req('POST', '/api/token', { seats: 2, pin: '1234', label: 'exponat-1' }, API_KEY);
  ...
  check('verify valid', verify.status === 200 && verify.json.valid === true && verify.json.seats === 2);

  const verifyBad = await req('POST', '/api/token/verify', { token: 'nope' });
  check('verify unknown token -> valid:false', verifyBad.status === 200 && verifyBad.json.valid === false);
  ...
  check('admin list enthaelt token', adminList.json.tokens.some((t) => t.token === token));

  const adminHealth = await req('GET', '/admin/health', null, ADMIN_KEY);
  check('admin health', adminHealth.status === 200 && adminHealth.json.ok === true && typeof adminHealth.json.rooms === 'number');
  ...
  check('admin ui served 200 html', uiKey.status === 200 && /text\/html/.test(uiKey.headers.get('content-type')) && /Admin/.test(uiKey.text));
  ...
  let rateLimited = 0, retryHeaders = 0;
  for (let i = 0; i < 8; i++) {
    const r = await req('POST', '/api/token', { seats: 1 }, API_KEY);
    if (r.status === 429) { rateLimited++; if (r.headers.get('retry-after')) retryHeaders++; }
  }
