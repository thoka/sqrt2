...
load(); setInterval(load, 5000);
	...
	function apiKey(req) {
		const b = bearer(req);
		return b && API_KEYS.has(b) ? b : null;
	}
			...
			if (p.startsWith('/admin/')) {
				...
				if (p === '/admin/tokens' && req.method === 'GET') {
					const list = [...tokens.entries()].map(([id, t]) => ({
						token: id,
						seats: t.seats,
						occupied: guestCount(id),
						pin: t.pin ? true : false,
						expiresAt: t.expiresAt,
						label: t.label,
					}));
	...
	function attachWs(httpServer) {
		...
		wss.on('connection', (ws, req) => {
			...
			const fail = (code, msg) => {
				try {
					ws.send(JSON.stringify({ type: 'error', code, message: msg }));
				} catch {
			...
			if (role === 'guest' && t.pin) {
				...
				if (locked > 0)
					return fail('pin_locked', `PIN-Versuche gesperrt für ${Math.ceil(locked / 1000)}s`);
				if (pin !== t.pin) {
					const wait = pinBackoff.registerFailure(tokenId);
					return fail(
						'pin_mismatch',
						wait > 0 ? `falsche PIN, Sperre ${Math.ceil(wait / 1000)}s` : 'wrong PIN',
					);
				}
			}
			if (role === 'guest' && guestCount(tokenId) >= t.seats)
				return fail('seats_exhausted', 'room full');

			let set = rooms.get(tokenId);
			if (!set) {
				set = new Set();
				rooms.set(tokenId, set);
			}
			set.add(connId);
			...
			ws.send(
				JSON.stringify({ type: 'joined', role, seats: t.seats, occupied: guestCount(tokenId) }),
			);
			...
			ws.on('close', () => {
				...
				if (set.size === 0) rooms.delete(tokenId);
					...
					broadcast(
						{ type: 'presence', event: 'leave', role, occupied: guestCount(tokenId) },
						true,
					);
			});
		});
...
if (isMain) {
	...
	server.listen(PORT, () => {
		...
		console.log(
			` ADMIN_KEY (nur einmal, in ${relay.dataDir ?? process.env.DATA_DIR ?? './data'}/admin_key):`,
		);
