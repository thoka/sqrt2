...
**Nächster Schritt (Stand 2026-07-15):** Phase 8 (Connection-Service-Anbindung) ist committet. Offen: Phase 6 (Politur), "Reine-Svelte"-Vertiefung (`bank-core.js`/`smoothing.js` → `src/lib/`, `RemoteControl` als Route foldbar) sowie E2E-Test der WS-Verbindung (Host startet Sitzung → Handy via QR join → beide Stores bleiben synchron; dafür läuft der Relay als `vite preview`-Begleitprozess, siehe `infra/connection-service`). Live-Relay-Tests der Protokoll-Stufen liegen bereits in `test-api.mjs`/`test-connection.mjs`.
