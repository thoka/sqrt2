...
**Nächster Schritt (Stand 2026-07-19):** Phase 8 (Connection-Service-Anbindung) ist committet. Flight-Morph (Flächenkonstanz + optionale 90°-Drehung beim Flug) implementiert auf Branch `better-morph`: `targetRot` wird beim Compile bestimmt (0, ±PI/2), `flightRotation`-Checkbox (an/aus) im Animations-Tab. Zwei Rendering-Passes: liegende Stücke zuerst (Rahmen via gridPath), fliegende obendrauf (Rahmen direkt im rotierten Kontext). Offen: Phase 6 (Politur), Bank-Flackern debuggen (Todo-Tastensteuerung hilft), "Reine-Svelte"-Vertiefung. E2E-Verifikation der WS-Verbindung liegt als **headless Integrationstest** `tests/relay/test-sqrt2-sync.mjs` vor.
