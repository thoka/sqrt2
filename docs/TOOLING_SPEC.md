...
**Status (2026-07-15):** Phasen 0–7 erledigt, Unit-Tests (99) grün, Build grün, E2E grün (inkl. Routing-Test: Vite `appType: 'mpa'` liefert 404 für unbekannte Pfade statt SPA-Fallback). `sqrt2.html` existiert nicht mehr – Einstiegspunkt ist `index.html` (Vite-Root). Neuer Nebenstrang **Connection-Service** (Zwei-Stufen-Auth, Cross-Device-Steuerung): siehe `docs/CONNECTION_SERVICE_SPEC.md` + `infra/connection-service/` (Relay-Prototyp mit TLS via `tailscale cert`).
...
**Nächster Schritt (Stand 2026-07-15):** Phase 7 ist committet (Branch `cleanup-by-hy3`). Offen: Phase 6 (Politur) sowie "Reine-Svelte"-Vertiefung (`bank-core.js`/`smoothing.js` → `src/lib/`, `RemoteControl` als Route foldbar). Neu dazugekommen: **Connection-Service** für Cross-Device-Steuerung – Prototyp in `infra/connection-service/`, Spec in `docs/CONNECTION_SERVICE_SPEC.md` (Anbindung an sqrt2: `configStore`/`playbackStore` über WS relayen, `BroadcastChannel` als Same-Browser-Fast-Path).
