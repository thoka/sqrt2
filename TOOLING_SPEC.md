...
| 4 | Canvas + HUD komponentisieren (`<TargetBankCanvas>`, `<RestCounterBars>`). `updateHUD()` in die zwei Widget-Varianten aufteilen (Balken jetzt, Grid neu) | mittel-hoch (neues Grid-Widget ist Neuentwicklung, nicht nur Port) | **teilweise** (4b `<RestCounterBars>` + 4c `<RestCounterGrid>` erledigt, siehe Stand) |
 ...
 **Nächster Schritt:** `<TargetBankCanvas>` (4a) - Canvas-Extraktion + rAF-Animationsschleife aus `sqrt2.html` in eine eigene Komponente auslagern (siehe renderFrame()/loop()/applyPlayback()). Das ist der hochriskante Teil von Phase 4: das Rendering hängt an Dutzenden Modul-Scope-Variablen und die rAF-Loop schreibt aktuell playbackStore, liest ihn via applyPlayback zurück - eine saubere Trennung braucht entweder, dass die Komponente ihren eigenen Loop+State hält und playbackStore nur noch als Schnittstelle nach außen (PlaybackBar/ControlPanel) nutzt, ODER dass renderFrame() als reine Funktion in ein `src/lib/render.js` ausgelagert wird, die Canvas + einen Render-State-Objekt nimmt. Ohne Browser hier kaum visuell prüfbar - daher bei der Umsetzung besonders auf Build + vitest (mit canvas-Mock) achten.
